<script lang="ts">
    ...
    const show = async () => {
        const res = await fetch(
            `/teacher/${$Userstore.email}/classroom/chat/api`,
            {
                method: "POST",
    ...
    const storechat = async (msg: string, t: string) => {
        ...
        let T = t;
        const res = await fetch(
            `/teacher/${$Userstore.email}/classroom/chat/api2`,
            {
                method: "POST",
    ...
    const ready = () => {
        conn!.on("data", (data: any) => {
            if (typeof data === "string") addMessage(data, "left", 's');
        });
